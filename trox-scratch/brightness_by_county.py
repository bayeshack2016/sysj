...
def clip_and_pull(vname, rname, dump=False):
    """
    Given `vname` the name of a vector (.shp) geo shape file and
    `rname` the name of a raster (.tif) geo image file, compute stats for the
    values in the image file within each shape.

    Code from @star_is_here
    """
    county = {}
